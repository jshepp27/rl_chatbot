...
from fn_lib import data, model, fn_utils
...
def run_test(test_data, net, end_token, device="cpu"):
    ...
    for p1, p2 in test_data:
        ...
        bleu_sum += fn_utils.calc_bleu(tokens, p2[1:])
        bleu_count += 1
...
if __name__ == "__main__":
    ...
    for epoch in range(MAX_EPOCHES):
        ...
        for batch in data.iterate_batches(train_data, BATCH_SIZE):
            # Zero out gradients at the start of each mini-batch (training) iteration
            # Explicitly set to zero prior to backpropogation (updating weights, biases)
            ...
            for idx, out_seq in enumerate(out_seq_list):
                ...
                if random.random() < TEACHER_PROB:
                    ...
                    r, seq = net.decode_chain_argmax(enc_item, out_seq.data[0:1],
                                                     len(ref_indices))
                    bleu_sum += fn_utils.calc_bleu(seq, ref_indices)
